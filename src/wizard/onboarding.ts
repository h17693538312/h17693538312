...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      "安全警告 - 请仔细阅读。",
      "",
      "Clawdbot 是一个业余项目，仍处于测试阶段。可能存在一些问题。",
      ...
      "- 对于任何带有工具或不可信收件箱的机器人，使用最强的可用模型。",
      "",
      "定期运行：",
      "clawdbot security audit --deep",
      "clawdbot security audit --fix",
      "",
      "必读文档：https://docs.clawd.bot/gateway/security",
    ].join("\n"),
    "安全",
  );

  const ok = await params.prompter.confirm({
    message: "我理解这是强大且存在固有风险的。是否继续？",
    initialValue: false,
  });
  if (!ok) {
...
export async function runOnboardingWizard(
  ...
  await prompter.intro("Clawdbot 引导设置");
  ...
  const quickstartHint = `稍后通过 ${formatCliCommand("clawdbot configure")} 配置详情。`;
